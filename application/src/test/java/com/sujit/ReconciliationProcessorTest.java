...
class ReconciliationProcessorTest {
    ...
    void givenSourceAndTargetFileWhenExecuteThenShouldCallReconciliationProcess() throws IOException {
        String source = System.getProperty("user.home") + File.separator + "clusus" + File.separator + "file1.csv";
        String target  = System.getProperty("user.home") + File.separator + "clusus" + File.separator + "file2.json";

        reconciliationProcessor.arrangeDataThenApplyReconciliation(source,target);
        
    }
    @Test
    void givenSourceAndTargetTxnListWhenReconcileThenShouldDetectMissingTxnAndSeparateThem() throws IOException {
        destinationDir = new File( "."+File.separator + "src" + File.separator+"test"+File.separator+"resources"+File.separator+"output");
        boolean isCreated = destinationDir.mkdir();if(!isCreated) {
        ...
        String expectedMatching = txn1.getTransId()
                        + COMMA
                        + txn1.getAmount()
                        + COMMA
        ...
        String expectedMissing1  = "SOURCE"
                + COMMA
                + txn3.getTransId()
                + COMMA
                + txn3.getAmount()
        ...
        String expectedMissing2  = "TARGET"
                + COMMA
                + txn5.getTransId()
                + COMMA
                + txn5.getAmount()
